...
impl StatsDemo {
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ui.heading("Statistics (machin-math)");
    ...
    fn compute(&mut self) {
        ...
        let mean = machin_math::stats::mean(&arr).unwrap_or(0.0);
        let variance = machin_math::stats::variance(&arr).unwrap_or(0.0);
        let std_dev = machin_math::stats::std_dev(&arr).unwrap_or(0.0);
        let median = machin_math::stats::median(&arr).unwrap_or(0.0);
        let (min, max) = machin_math::stats::min_max(&arr).unwrap_or((0.0, 0.0));
        let sample_var = machin_math::stats::sample_variance(&arr).ok();
