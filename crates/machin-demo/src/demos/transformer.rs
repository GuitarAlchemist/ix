...
impl TransformerDemo {
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ui.heading("Transformer (machin-nn)");
    ...
    fn run_attention(&mut self) {
        use machin_nn::attention::{causal_mask, multi_head_attention};
        use rand::Rng;
    ...
    fn run_positional(&mut self) {
        match self.pe_type {
            PeType::Sinusoidal => {
                let pe = machin_nn::positional::sinusoidal_encoding(self.pe_max_len, self.pe_d_model);
            ...
            PeType::Rope => {
                ...
                let rotated = machin_nn::positional::rope_rotate(&base_vec, 10000.0);
            ...
            PeType::Alibi => {
                let n_heads = 4;
                let slopes = machin_nn::positional::alibi_slopes(n_heads);
                ...
                self.pe_data = slopes.iter().map(|&slope| {
                    let bias = machin_nn::positional::alibi_bias(seq, slope);
                    let last_row = seq - 1;
    ...
    fn run_full_block(&mut self) {
        use machin_nn::transformer::TransformerStack;
        use machin_nn::attention::causal_mask;
