...
- `CountMinSketch::with_error(epsilon, delta)` ceils width to `e/epsilon` and depth to `ln(1/delta)`; the actual error guarantee holds with probability `>= 1 - delta`. Inputs are not validated. `epsilon` and `delta` must be finite positive values; `delta = 0` computes infinite depth before casting/clamping and can trigger pathological allocation instead of a safe fallback.
