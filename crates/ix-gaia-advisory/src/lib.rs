...
pub enum OutOfSampleStatus {
    ...
    /// consumption ledger.
    ///
    /// There is exactly one construction site — [`measured_from_earned_holdout`]
    /// — and reaching it requires an [`EarnedHoldout`], which
    /// [`EarnedHoldout::admit`] issues only when every earned condition is
    /// *measured*. NC-20 fails the suite if a second site ever appears, or if
    /// the one site moves out of that function.
    ///
...
///
/// The field is private and the only constructor measures the corpus root, so
/// no caller can hand one in. That is the whole content of NC-20a's third
/// condition: a digest supplied by the lane under audit certifies nothing,
/// because that lane could supply the digest of any corpus it liked.
#[derive(Debug, Clone, PartialEq, Eq)]
struct MeasuredCorpusDigest(String);
...
impl MeasuredCorpusDigest {
    ...
    // M3-as-selected calls it; the colocated boundary tests measure their own
    // scratch roots, so the constructor is exercised rather than asserted.
    #[allow(dead_code)]
    fn measure(corpus_root: &std::path::Path) -> Result<Self, AdvisoryRefusal> {
...
/// [`MeasuredCorpusDigest`]: a `report_digest` a caller states is a claim about
/// a report, and a claim cannot certify the report it is a claim about.
#[derive(Debug, Clone, PartialEq, Eq)]
struct MeasuredReportDigest(String);

impl MeasuredReportDigest {
    /// The only constructor. Refuses any relative path but the canonical one.
    // Reached by S12; no path in M3-as-selected calls it.
    #[allow(dead_code)]
    fn recompute(relative_path: &str, report_bytes: &[u8]) -> Result<Self, AdvisoryRefusal> {
...
/// The type exists so [`EarnedHoldout::admit`] can require a receipt *by type*
...
///
/// A value of this type **is** the proof. It is obtainable only from
/// [`EarnedHoldout::admit`], which measures each condition rather than
/// accepting a claim about it, and holding one is the only thing that entitles
/// a run to a `Measured` out-of-sample status.
#[derive(Debug, Clone, PartialEq, Eq)]
struct EarnedHoldout {
    corpus_digest_ix_agg_1: String,
    report_digest: String,
}

impl EarnedHoldout {
    /// The admission gate. Total, and with no discretion in it.
    ///
    /// * `Ok(None)` — an earning condition is absent, so the run is not
    ///   admitted and emits `Unknown` (NC-20a, NC-20b). Fail-closed.
    /// * `Err(ReplayDivergence)` — the receipt's `report_digest` does not equal
    ///   the digest recomputed from the report actually emitted (NC-20c). The
    ///   run is `Void` and produces **no artifact**.
    ...
    fn admit(
        ...
        // NC-20a(i) / NC-20b — the class firewall. `Development`, `Selection`
        // and a class-less run can never reach `Measured`.
        ...
        // measured itself, at this candidate's fingerprint and swept width. A
        // caller-supplied digest never reaches this comparison: `corpus` can
        // only have come from `MeasuredCorpusDigest::measure`.
        ...
        // NC-20c — no self-certification. The digest the receipt carries must
        // equal the one recomputed from the report that was actually emitted.
...
/// [`EarnedHoldout`], which requires [`EarnedHoldout::admit`], which measures
/// every §10.3 condition. There is no other way to obtain the variant.
fn measured_from_earned_holdout(earned: &EarnedHoldout) -> OutOfSampleStatus {
    OutOfSampleStatus::Measured {
        corpus_digest_ix_agg_1: earned.corpus_digest_ix_agg_1.clone(),
        report_digest: earned.report_digest.clone(),
    }
}
...
mod oos_emission {
    ...
    fn matching_receipt(corpus: &MeasuredCorpusDigest, window_bytes: u32) -> LedgerReceipt {
        LedgerReceipt {
            ...
            report_digest: canonical_report().0,
        }
    }
    ...
    fn nc_20a_all_earned_conditions_present_yields_measured() {
        ...
        for window_bytes in SWEPT_WIDTHS {
            ...
            assert!(
                json.contains(&corpus.0),
                "the measured corpus digest is carried"
            );
            assert!(
                json.contains(&report.0),
    ...
    fn nc_20a_admission_is_exhaustive_over_its_conditions() {
        ...
        let other = MeasuredCorpusDigest(digest::sha256_hex(b"a corpus this run never measured"));
        let report = canonical_report();
    ...
    fn nc_20a_a_caller_supplied_corpus_digest_never_self_certifies() {
        ...
        assert_ne!(forged.corpus_digest_ix_agg_1, corpus.0);
