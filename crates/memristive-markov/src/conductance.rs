...
fn serialize_conductances<S: Serializer>(
    map: &HashMap<(Vec<usize>, usize), f64>,
    serializer: S,
...
pub struct ConductanceMatrix {
    #[serde(
        serialize_with = "serialize_conductances",
        deserialize_with = "deserialize_conductances"
    )]
...
impl ConductanceMatrix {
    ...
    pub fn set(&mut self, context: &[usize], next: usize, g: f64) {
        self.conductances
            .insert((context.to_vec(), next), g.max(self.g_min));
    }
    ...
    pub fn len(&self) -> usize {
        self.conductances.len()
    }
    pub fn is_empty(&self) -> bool {
        self.conductances.is_empty()
    }
}
