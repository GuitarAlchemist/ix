...
14. **Four behavioural controls now cover what the grep never could** *(post-freeze, blocker B2)*. The grep
    ...
    **Test count: 70 → 85** — 70 inherited, unchanged and still green; **10** colocated boundary unit tests in
    `src/lib.rs`, which exist there because the behaviour is deliberately unreachable through the closed
    public Holdout seam; **5** public-seam tests in `tests/b2_oos_emission.rs`.
