...
| E7 | SVM, random forest, attention, transformer | ix-supervised / ix-nn | `crates/ix-supervised/src/{svm,decision_tree}.rs`; `crates/ix-nn/src/{attention,transformer,positional}.rs` | already_done | document | P3 | XS | high | partial | no | no | MCP `ix_random_forest`, `ix_supervised`, `ix_gradient_boosting`, `ix_nn_forward`. Four more of the 20 closures, all covered. |
