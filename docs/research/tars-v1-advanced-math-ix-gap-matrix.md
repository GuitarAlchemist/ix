...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A1 | Linear state-space model `x_{k+1}=Ax_k+Bu_k+w_k` | ix-signal / new | — | missing_algorithm | prototype | P1 | S | high | no | no | no | **Name-collision trap.** `crates/ix-graph/src/state_space.rs` is *not* this: it defines `trait State`, `astar()` and `beam_search()` — a discrete search space, not an LTI system. A grep for `state_space` returns that as a false positive. `crates/ix-signal/src/kalman.rs` carries `A`/`B` internally but exposes no reusable `StateSpaceModel`. |
| A2 | Non-linear state-space `x_{k+1}=f(x_k,u_k)` | ix-dynamics | `crates/ix-dynamics/src/neural_ode.rs` | partially_implemented | prototype | P2 | M | medium | no | no | no | Neural ODE covers the *continuous-time* learned-`f` case. The discrete-time analytic-`f` case A1 needs is absent. Build A1 first; A2 is then a thin wrapper. |
...
| A5 | Observability & controllability analysis | ix-signal / new | — | missing_algorithm | prototype | P1 | XS | high | no | no | no | **Best first tracer-bullet in this matrix.** Kalman rank / Popov–Belevitch–Hautus tests are a few dozen lines over `ndarray`, and the oracle is exact: textbook `(A,B)` pairs with known rank deficiency. All grep hits for `observability` are `ix-governance`/`ix-context` telemetry, unrelated. |
| A6 | Lyapunov stability analysis | ix-chaos | `crates/ix-chaos/src/lyapunov.rs` | partially_implemented | prototype | P2 | M | high | no | no | no | **Two different Lyapunovs.** IX has the Lyapunov *exponent*, a chaos diagnostic exposed as MCP `ix_chaos_lyapunov`. TARS asks for a Lyapunov *function* / stability certificate for a state-space model — solving `AᵀPA − P = −Q` for `P ≻ 0`. The discrete Lyapunov equation is testable against known stable/unstable `A`. Depends on A1. |
...
| B1b | Persistent homology over a DuckDB column | ix-duck | `crates/ix-duck/sql/persistence_h0.sql` — `ix_persistence_h0()`, `ix_betti_0_at(radius)`, `ix_topo_violations()` | implemented_needs_exposure | prototype | P2 | S | high | **yes** (SQL macro) | no | no | **Amended 2026-09-08 — see [`2026-09-08-code-topology-over-ix.md`](2026-09-08-code-topology-over-ix.md) §7.** Delivered as a SQL macro, **not** a UDF: `pareto_frontier.sql` records that ix-duck's `duck`/`udf` features are never CI-compiled, so a UDF would be invisible to CI. Note this row's `duckdb` column therefore departs from the [convention above](#how-to-read-the-matrix), which counts only registered UDFs — the convention is what should change. Scope is H0 only, and that is a property of the input, not a shortcut: one column is a point cloud in R^1, where every H1 class an unfilled Rips graph reports is an artefact of the dimension cap. The `ix_bottleneck(a,b)` scalar form the original note suggested is **deliberately not** delivered: `ix_topo::bottleneck_distance` documents itself as a greedy approximation, so a SQL surface over it would freeze an approximation as a contract. Cross-checked against the general engine (`rips_complex` + `compute_persistence`) on a frozen golden, per the two-surface shape of #294; the DuckDB CLI half of that check is opportunistic and does not run on a CI machine without `duckdb` on PATH. |
...
| # | candidate_name | ix_area | current_ix_surface | status | recommended_action | prio | cx | testability | duckdb | skill | pipeline | notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F1 | Topological code analysis (persistent homology over code structure) | ix-topo × ix-code | `crates/ix-topo/` + `crates/ix-code/`; MCP `ix_code_analyze`, `ix_ast_query`, `ix_code_smells`; DuckDB `ix_code_complexity`, `ix_code_metrics`, `ix_code_smells`, `ix_ast_query` | implemented_needs_exposure | prototype | P2 | M | medium | partial | no | **candidate** | **Best-supported composition in this section.** **Amended 2026-09-08 — see [`2026-09-08-code-topology-over-ix.md`](2026-09-08-code-topology-over-ix.md):** the composition already existed on `main` as `crates/ix-code/src/topology.rs` (Rips filtration over an inverted-weight call graph, feeding `ix_topo::persistence::compute_persistence`), but behind ix-code's `topology` feature, which **no crate in the workspace enabled** — so it was never compiled or tested by CI, the same invisibility class `pareto_frontier.sql` records for ix-duck's `duck`/`udf`. The genuine gap was the converter: `topology::CallGraph` and `semantic::CallGraph` are distinct types and nothing mapped between them, so `compute_code_topology` had zero callers outside its own tests. Now joined and exposed as MCP `ix_code_topology`. Measured over all 82 IX crates. Caveat that survived the measurement: with the filtration capped at dim 1, `betti_1` **equals** the circuit rank `E - V + betti_0` (verified on all 82) — i.e. McCabe on the dependency graph, not new information. The part beyond McCabe is the H0 persistence. Both halves are built and already exposed *separately*; nothing joined them. IX also has the fuel TARS lacked: `crates/ix-code` (the single crate CLAUDE.md refers to as `ix-code-*`) owns offline corpus AST analysis under CLAUDE.md's realtime/offline boundary. Testability is `medium`, not `high`: "does code topology predict anything" needs a labelled outcome, and the honest baseline is `ga/state/quality/`. Note the TARS target file `CodeAnalyzerService.fs` does not exist at the pinned SHA — the idea is salvage, the implementation is not. |
...
| status | count | rows |
| --- | ---: | --- |
| `already_done` | 19 | B1, B2, B5, C1, C3, C4, D1, D3, D4, D5, D7, D8, E1, E4, E5, E6, E7, E9, E10 |
| `missing_algorithm` | 13 | A1, A4, A5, A7, B6, C5, C6, C7, D2, D6, E2, E3, E8 |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Discrete `StateSpaceModel` + observability/controllability rank tests in `ix-signal` | A1, A5 | Unblocks all of section A and #193; the smallest slice that makes "state-space" mean the control-theory thing in IX | Textbook `(A,B,C)` triples with known rank deficiency; exact | S |
